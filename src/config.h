...
// updates them. If the files are already up-to-date, false is returned and
// nm_err is cleared. If the files were updated, true is returned and nm_err is
// cleared. If *files is NULL, it is equivalent to doing `*files =
// nm_config_files()`. If an error occurs, the pointer is left untouched and
// false is returned with nm_err set. Warning: if the files have changed, the
// pointer passed to files will become invalid (it gets replaced).
...
// returned and nm_err is set. On success, the config is returned and nm_err is
// cleared.
